import { signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, getRedirectResult } from 'firebase/auth'
import { useState, useEffect } from 'react'
import { auth, googleProvider } from '../firebase/config'

export default function Login() {
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [emailMode, setEmailMode] = useState(false)
  const [isRegister, setIsRegister] = useState(false)

  useEffect(() => {
    getRedirectResult(auth)
      .then(() => setLoading(false))
      .catch(err => {
        console.error('redirect result error:', err)
        setLoading(false)
      })
  }, [])

  const handleGoogleLogin = async () => {
    setError(null)
    setLoading(true)
    try {
      await signInWithPopup(auth, googleProvider)
    } catch (err) {
      console.error('google error:', err)
      if (err.code !== 'auth/popup-closed-by-user') {
        setError(`Błąd Google: ${err.code}`)
      }
      setLoading(false)
    }
  }

  const handleEmailLogin = async (e) => {
    e.preventDefault()
    setError(null)
    setLoading(true)
    try {
      if (isRegister) {
        await createUserWithEmailAndPassword(auth, email, password)
      } else {
        await signInWithEmailAndPassword(auth, email, password)
      }
    } catch (err) {
      console.error('email error:', err)
      const msgs = {
        'auth/user-not-found': 'Nie znaleziono konta z tym emailem.',
        'auth/wrong-password': 'Złe hasło.',
        'auth/email-already-in-use': 'Konto z tym emailem już istnieje.',
        'auth/weak-password': 'Hasło za krótkie (min. 6 znaków).',
        'auth/invalid-email': 'Nieprawidłowy format emaila.',
        'auth/invalid-credential': 'Nieprawidłowy email lub hasło.',
      }
      setError(msgs[err.code] || `Błąd: ${err.code}`)
      setLoading(false)
    }
  }

  return (
    <div className="login-screen">
      <div className="login-card">
        <div className="login-logo">
          <span className="login-logo-dot" />
          <span className="login-logo-dot" />
          <span className="login-logo-dot" />
          <span className="login-logo-dot" />
        </div>
        <h1 className="login-title">Mój Świat</h1>
        <p className="login-subtitle">Budżet · Nawyki · Modlitwa · Zadania</p>

        {!emailMode ? (
          <>
            <button className="btn-google" onClick={handleGoogleLogin} disabled={loading}>
              {loading ? <span className="btn-spinner" /> : (
                <svg width="18" height="18" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
              )}
              {loading ? 'Ładowanie...' : 'Zaloguj się przez Google'}
            </button>
            <button className="btn-email-toggle" onClick={() => { setEmailMode(true); setError(null) }}>
              lub użyj email i hasła
            </button>
          </>
        ) : (
          <form onSubmit={handleEmailLogin} className="login-email-form">
            <input
              className="login-input"
              type="email"
              placeholder="Email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              required
              autoFocus
            />
            <input
              className="login-input"
              type="password"
              placeholder="Hasło"
              value={password}
              onChange={e => setPassword(e.target.value)}
              required
            />
            <button className="btn-email-submit" type="submit" disabled={loading}>
              {loading ? <span className="btn-spinner" style={{ borderColor: '#ccc', borderTopColor: 'var(--bg)' }} /> : (isRegister ? 'Utwórz konto' : 'Zaloguj się')}
            </button>
            <div className="login-toggle-row">
              <button type="button" className="btn-email-toggle" onClick={() => setIsRegister(r => !r)}>
                {isRegister ? 'Mam już konto' : 'Utwórz nowe konto'}
              </button>
              <button type="button" className="btn-email-toggle" onClick={() => { setEmailMode(false); setError(null) }}>
                ← Wróć
              </button>
            </div>
          </form>
        )}

        {error && <p className="login-error">{error}</p>}
        <p className="login-note">Twoje dane są prywatne i widoczne tylko dla Ciebie</p>
      </div>
    </div>
  )
}
